<script lang="ts">
  ...
  let filteredItems = $derived(
    gameState.inventory.getFilteredItems(selectedType, selectedRarity)
  );

  let availableTypes = $derived(gameState.inventory.getAvailableTypes());
  let availableRarities = $derived(gameState.inventory.getAvailableRarities());
...
<div class="inventory-grid rounded-xl border border-gray-700 bg-gray-900 p-4 sm:p-6 lg:p-8">
  ...
  <div class="mt-6 text-xs text-gray-400">
    ...
    <div class="mt-1">
      Items: {filteredItems.length}{gameState.inventory.size !== filteredItems.length ? ` of ${gameState.inventory.size}` : ''}
    </div>
  </div>
</div>